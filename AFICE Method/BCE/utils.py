...
@dataclass
class MyGenerateBeamDecoderOnlyOutput(ModelOutput):
    """
    My outputs of decoder-only generation models, when using beam methods.
...
class GenerationMixin:
    ...
    def generate(
        ...
        **kwargs,
        ...
        """
        # 打开一个文件用于写入
        with open('/storage_fast/ygzhao/FastChat/fastchat/llm_judge/output1.txt', 'w') as file:
    ...
    def _greedy_search(
        ...
        **model_kwargs,
        ...
        while self._has_unfinished_sequences(this_peer_finished, synced_gpus, device=input_ids.device):
            ...
            probs = F.softmax(next_tokens_scores, dim=-1)

            my_scores.append(probs.gather(dim=-1, index=next_tokens.unsqueeze(-1)).squeeze(-1))
    ...
    def _sample(
        ...
        **model_kwargs,
        ...
        while self._has_unfinished_sequences(this_peer_finished, synced_gpus, device=input_ids.device):
            ...
            # 使用 next_tokens 从 probs 中取出对应的概率
            selected_probs = probs[torch.arange(probs.size(0)), next_tokens]
            print('selected_probs', selected_probs, 'next_tokens', next_tokens)
            my_scores.append(selected_probs)
    ...
    def _beam_search(
        ...
        **model_kwargs,
        ...
        while self._has_unfinished_sequences(this_peer_finished, synced_gpus, device=input_ids.device):
            ...
            beam_search_list.append(beam_scores.tolist())
        ...
        if return_dict_in_generate:
            ...
            if self.config.is_encoder_decoder:
                ...
                return MyGenerateBeamDecoderOnlyOutput(
                    sequences=sequence_outputs["sequences"],
                    sequences_scores=sequence_outputs["sequence_scores"],
                    scores=scores,
                    logits=raw_logits,
                    ...
                    hidden_states=decoder_hidden_states,
                    past_key_values=model_kwargs.get("past_key_values"),
                    beam_scores_list=beam_search_list,
                )
        else:
    ...
    def _beam_sample(
        ...
        **model_kwargs,
        ...
        while self._has_unfinished_sequences(this_peer_finished, synced_gpus, device=input_ids.device):
            ...
            beam_next_tokens = beam_outputs["next_beam_tokens"]
            beam_idx = beam_outputs["next_beam_indices"]
            beam_sample_list.append(beam_scores.tolist())
        ...
        if return_dict_in_generate:
            ...
            if self.config.is_encoder_decoder:
                ...
                return MyGenerateBeamDecoderOnlyOutput(
                    ...
                    hidden_states=decoder_hidden_states,
                    past_key_values=model_kwargs.get("past_key_values"),
                    beam_scores_list=beam_sample_list,
                )
        else:
            return sequence_outputs["sequences"]
